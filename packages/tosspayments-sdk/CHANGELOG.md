...
- 13f9ad0: `clearTossPayments` 가 default URL 뿐 아니라 `loadTossPayments` 가 시도한 모든 src 의 스크립트를 정확히 제거하도록 수정합니다. 패키지 모듈 내부에 (src, namespace) 추적 state 를 두어, custom `src` 로 load 한 케이스에서도 자연스럽게 해당 스크립트와 sdk-loader 의 cache entry 가 함께 정리됩니다.
